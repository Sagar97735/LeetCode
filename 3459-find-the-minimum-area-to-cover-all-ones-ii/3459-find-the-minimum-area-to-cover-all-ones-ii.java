...
// private:
//     // divideGrid: current grid ko "parts" (kitne rectangles chahiye) me todta hai
//     int divideGrid(vector<vector<int>>& grid, int parts, int top, int bottom, int left, int right) {
//         if (top > bottom || left > right) return 0;

//         // Base case: agar sirf 1 rectangle chahiye
//         if (parts == 1) {
//             return minRectangleArea(grid, top, bottom, left, right);
//         }
...
//     // minRectangleArea: ek rectangle ka minimum bounding area return karta hai
//     int minRectangleArea(vector<vector<int>>& grid, int top, int bottom, int left, int right) {
//         int minCol = right;
//         int maxCol = left;
//         int minRow = bottom;
//         int maxRow = top;
...
//         if (!foundOne) return 0; // agar 1 hi nahi mila
//         return (maxCol - minCol + 1) * (maxRow - minRow + 1);
//     }
// };
